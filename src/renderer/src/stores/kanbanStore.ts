...
export function updateColumn(payload: Pick<Column, 'name' | 'columnId'>) {
  let column = STORE.value.find((column) => column.columnId === payload.columnId)
  if (column) {
...
export function updateTask(columnId: Column['columnId'], task: Task) {
  ...
  let updatedTasks = column.tasks.map((oldTask) => {
...
export function moveTask(
  ...
  if (targetTaskId) {
    let targetTaskIndex = targetColumn?.tasks.findIndex((task) => task.taskId === targetTaskId)
...
export function moveColumn(columnId: Column['columnId'], targetColumnId: Column['columnId']) {
  ...
  STORE.value = STORE.value.filter((column, columnIndex) => {
